$(function(){
        var zoom = new ZoomView('#slide_menu', '#mobile_right_nav');
    });
    ...
    function DragView(target) {
      ...
      this.OnDrag = function(event) {
        ...
        for(var t=0; t<touches.length; t++) {
          ...
          if(el && el == this.target) {
            var menu_right_pos = $("#slide_menu").position();
            this.drag.push({
              el: el,
              pos: event.touches[t]
            });
      ...
      this.OnDragEnd = function(event) {
            ...
            var menu_right_pos = target.position();
            windowWidth = $(window).width();
            var thirty_percent = windowWidth * 0.3;
            var sixty_percent = windowWidth * 0.5;
            if(menu_right_pos.left <= sixty_percent) {
                target.animate({left:thirty_percent+"px"}, 300);
            } else {
                target.animate({left:windowWidth+"px"}, 300);
            }
      }
    }
